...
func (s *Server) GetThumbnail(ctx context.Context, req *proto.GetThumbnailRequest) (*proto.Thumbnail, error) {
    ...
    if err != nil {
        s.logger.Info("failed to convert thumbnail to proto", slog.String("error", err.Error()))
        return nil, status.New(codes.Unknown, err.Error()).Err()
    }

    s.logger.Info(
        "successful response: %v",
        slog.String("video_id", resp.VideoId),
        slog.String("quality", thumb.Quality.String()),
    )
    return resp, nil
}
